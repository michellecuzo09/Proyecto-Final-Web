...
export class ActualizarGradoModalComponent implements OnInit {
  ...
  constructor(public modalRef: BsModalRef, private fb: FormBuilder, private GService: GradoOcupacionalService) { }
  ...
  loadDetails() {
    if (this.grado_Id) {
      ...
      this.GService.getgradoid(this.grado_Id).subscribe(grado => {
  ...
  onSubmit() {
    if (this.updateForm && this.updateForm.valid) {
      ...
      console.log('Jornada ID seleccionado:', updatedGrado.grado_id);
      if (!updatedGrado.grado_id) {
        console.error('Error: ID de grado no válido');
        return;
      }
  
      this.GService.updateJornada(updatedGrado).subscribe(
        data => {
          console.log('Grado Ocupacional actualizada con éxito:', data);
          this.modalRef.hide();  // Cierra la ventana desplegable después de la actualización
        },
        error => {
          console.error('Error al actualizar el grado ocupacional:', error);
